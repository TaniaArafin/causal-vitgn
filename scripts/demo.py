...
def cascade_card(idx: int, sample: dict, num_users: int):
    ...
    neighbors = [int(u) for u in sample["neighbors"] if int(u) != 0]
    activated = float(sample["activated"])

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Cascade ID", f"#{idx}")
    col2.metric("Target user", int(sample["target_node"]))
...
def main():
    ...
    neighbors = sorted({int(u) for u in sample["neighbors"] if int(u) != 0})
    target = int(sample["target_node"])
    removable = [target] + [u for u in neighbors if u != target]

    if not removable:
        st.warning("No users available to intervene on for this cascade.")
        st.stop()
