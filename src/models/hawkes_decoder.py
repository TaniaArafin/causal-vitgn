...
class NeuralHawkesDecoder(nn.Module):
   ...
   def forward(
       ...
       """Returns the unbounded logit l of activation probability.


       Downstream, the loss converts this to lambda via softplus when it
       needs a Hawkes-style intensity, but trains on the raw logit using
       binary cross-entropy with logits. Returning the raw logit avoids
       the gradient-killing clamps in the loss and lets the model learn
       ...
       return self.intensity_net(x).squeeze(-1)
