...
class VariationalEncoder(nn.Module):
   ...
   def __init__(
       ...
       # Initialise the final logvar layer so sigma ~= 0.14 at start
       # (logvar = -4). With sigma << 1, the sampled z is dominated by mu,
       # not noise, so the decoder receives a usable signal from epoch 0.
       # Without this, the encoder cannot escape posterior collapse because
       # gradients through reparameterised noise have huge variance early on.
       nn.init.zeros_(self.logvar_net[-1].weight)
